...
from ..models import BorrowSnapshot, FourHourBar, Split, Stock, SyncRun
...
router=APIRouter();RANGE_START=date(2026,5,1);RANGE_END=date(2026,9,30);BORROW_SYNC_STATUS={"running":False,"processed":0,"total":0,"saved":0,"not_found":0,"errors":[],"last_finished":None};BORROW_SYNC_CONCURRENCY=32
...
def _latest_split_rows(db,include_future=False):
    q=select(Split,Stock).join(Stock,Stock.id==Split.stock_id).where(Split.effective_date>=RANGE_START,Split.effective_date<=RANGE_END)
    if not include_future:q=q.where(Split.effective_date<=date.today())
...
async def run_borrow_sync():
    ...
    try:
        ...
        rows=_latest_split_rows(db);targets={s.symbol:(sp,s) for sp,s in rows};BORROW_SYNC_STATUS["total"]=len(targets);run.total=len(targets);db.commit();sem=asyncio.Semaphore(BORROW_SYNC_CONCURRENCY);limits=httpx.Limits(max_connections=64,max_keepalive_connections=40)
        async with httpx.AsyncClient(timeout=30,follow_redirects=True,headers={"User-Agent":"Mozilla/5.0 QanasDataEngine/1.0","Cache-Control":"no-cache","Pragma":"no-cache"},limits=limits) as client:
            tasks=[asyncio.create_task(_borrow_fetch_one(client,sem,symbol,s.exchange)) for symbol,(sp,s) in targets.items()]
            for task in asyncio.as_completed(tasks):
                symbol,snap,err=await task;sp,stock=targets[symbol];BORROW_SYNC_STATUS["processed"]+=1;run.processed+=1
                if err:BORROW_SYNC_STATUS["errors"].append({"symbol":symbol,"exchange":stock.exchange,"error":err[:180]})
                elif not snap:BORROW_SYNC_STATUS["not_found"]+=1;run.not_found+=1;BORROW_SYNC_STATUS["errors"].append({"symbol":symbol,"exchange":stock.exchange,"error":"borrow data not found"})
                else:
                    if not stock.exchange and snap.get("exchange"):stock.exchange=snap["exchange"]
...
async def sync_borrow_symbol(symbol:str,db:Session=Depends(get_db)):
    ...
    snapshot=await fetch_borrow_snapshot(symbol,exchange=stock.exchange)
    if not snapshot:raise HTTPException(status_code=404,detail={"message":"borrow data not found","symbol":symbol,"exchange":stock.exchange})
    if not stock.exchange and snapshot.get("exchange"):stock.exchange=snapshot["exchange"]
    ...
    db.commit();return {"symbol":symbol,"exchange":stock.exchange,"available":row.available_shares,"ctb":row.fee_rate,"fee_rate":row.fee_rate,"rebate_rate":row.rebate_rate,"source":row.source,"timestamp":row.ts,"source_reported_at":snapshot.get("reported_at"),"source_url":snapshot.get("url")}
@router.get("/borrow/{symbol}/history")
def borrow_history(symbol:str,limit:int=100,db:Session=Depends(get_db)):
    symbol=symbol.upper().strip();stock=db.scalar(select(Stock).where(Stock.symbol==symbol))
    if not stock:raise HTTPException(status_code=404,detail="symbol not found")
    rows=db.scalars(select(BorrowSnapshot).where(BorrowSnapshot.stock_id==stock.id).order_by(BorrowSnapshot.ts.desc(),BorrowSnapshot.id.desc()).limit(max(1,min(limit,1000)))).all();return [{"timestamp":r.ts,"available":r.available_shares,"ctb":r.fee_rate,"fee_rate":r.fee_rate,"rebate_rate":r.rebate_rate,"source":r.source} for r in rows]
@router.get("/splits")
...
def stock_detail(symbol:str,db:Session=Depends(get_db)):
    row=db.execute(select(Split,Stock).join(Stock,Stock.id==Split.stock_id).where(Stock.symbol==symbol.upper().strip(),Split.effective_date>=RANGE_START,Split.effective_date<=RANGE_END).order_by(Split.effective_date.desc(),Split.id.desc()).limit(1)).first()
    if not row:raise HTTPException(status_code=404,detail="symbol not found")
    return _split_payload(db,*row)
@router.get("/hunt")
