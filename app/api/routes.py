...
from ..services.sync_prices import run_price_sync, PRICE_SYNC_STATUS
...
async def sync_prices_now(background_tasks: BackgroundTasks):
    if PRICE_SYNC_STATUS["running"]:
        return {"started": False, "status": PRICE_SYNC_STATUS}
    background_tasks.add_task(run_price_sync, date(2026, 6, 1), date(2026, 9, 30))
    return {"started": True, "status_url": "/api/sync/prices/status"}
