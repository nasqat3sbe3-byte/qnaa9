...
*{box-sizing:border-box}body{margin:0;background:#070b0e;color:#eef2f3;font-family:Tahoma,Arial,sans-serif;padding-bottom:62px}.w{max-width:720px;margin:auto;padding:9px}.head{height:42px;display:flex;align-items:center;justify-content:space-between}.logo{font-size:23px;font-weight:900;letter-spacing:-1px}.logo i{font-style:normal;color:#35dc98}.status{font-size:10px;color:#35dc98}.filters{display:flex;gap:5px;overflow:auto;padding:5px 0 8px;scrollbar-width:none}.f{border:1px solid #1a282e;background:#0b1216;color:#aab7bb;border-radius:7px;padding:6px 9px;white-space:nowrap;font-size:11px}.f.on{background:#173028;border-color:#2c7358;color:#64e8aa;font-weight:800}.count{font-size:10px;color:#64747a;margin:0 2px 6px}.card{background:#0b1115;border:1px solid #172228;border-radius:9px;padding:9px 10px;margin-bottom:6px;box-shadow:0 4px 18px #0004}.top{display:flex;justify-content:space-between;align-items:center}.sym{font-size:16px;font-weight:900}.score{font-size:14px;color:#5ce6a7;font-weight:900}.badges{display:flex;gap:4px;align-items:center}.badge{font-size:9px;padding:3px 5px;border-radius:5px;background:#142127;color:#91a3a8}.gold{color:#efc36b}.red{color:#ff8177}.line{display:grid;grid-template-columns:1.05fr 1fr 1fr 1fr;gap:5px;margin-top:7px}.kv{min-width:0}.kv small{display:block;color:#5f7076;font-size:8px;margin-bottom:2px}.kv b{display:block;font-size:11px;overflow:hidden;text-overflow:ellipsis}.price{color:#eef2f3}.good{color:#5ce6a7}.more{display:none;border-top:1px solid #172228;margin-top:7px;padding-top:7px}.card.open .more{display:grid}.events .card{padding:9px}.eventtxt{font-size:12px;margin-top:4px}.time{font-size:9px;color:#5f7076}.empty{text-align:center;color:#607077;padding:28px}.nav{position:fixed;bottom:0;left:0;right:0;background:#090f12f5;border-top:1px solid #172228;z-index:5}.navin{max-width:720px;margin:auto;display:grid;grid-template-columns:1fr 1fr}.nav button{border:0;background:none;color:#69797e;padding:12px;font-size:12px}.nav button.on{color:#5ce6a7;font-weight:900}.hidden{display:none}.section{font-size:10px;color:#64747a;margin:5px 2px 8px}@media(min-width:620px){.cards{display:grid;grid-template-columns:1fr 1fr;gap:6px}.card{margin:0}.events .card{margin-bottom:6px}}</style></head><body><div class="w"><div class="head"><div class="logo"><i>〽</i> نبض</div><div id="hb" class="status">● اتصال...</div></div>
<section id="radar"><div class="filters"><button class="f on" data-f="all">الكل</button><button class="f" data-f="ready">🎯 الأجهز</button><button class="f" data-f="near">قريب</button><button class="f" data-f="zero">0 شورت</button><button class="f" data-f="momentum">⚡ لحظي</button><button class="f" data-f="top">👑 TOP</button></div><div id="count" class="count"></div><div id="cards" class="cards"></div></section>
<section id="pulse" class="hidden events"><div class="section">آخر الأحداث · HALT · الأخبار الإيجابية</div><div id="events"></div></section></div><div class="nav"><div class="navin"><button class="on" data-page="radar">🎯 الرادار</button><button data-page="pulse">🔔 النبض</button></div></div><script>
let snap={},evs=[],halts={},news={},filter='all';const N=v=>v==null?'—':Number(v).toLocaleString('en-US',{maximumFractionDigits:2}),P=v=>v==null?'—':Number(v).toFixed(1)+'%',D=v=>v==null?'—':'$'+Number(v).toFixed(Number(v)<1?4:2);async function J(x){let r=await fetch(x,{cache:'no-store'});if(!r.ok)throw Error(r.status);return r.json()}
function A(){return Object.values(snap.rows||{}).map(x=>({...x,...(x.signal||{}),q:x.price||{},br:x.borrow||{}}))}function F(){let a=A();if(filter==='ready')a=a.filter(x=>x.ready);if(filter==='near')a=a.filter(x=>x.near_ready&&!x.ready);if(filter==='zero')a=a.filter(x=>Number(x.br.available??x.available)===0);if(filter==='momentum')a=a.filter(x=>x.ignition?.fresh);if(filter==='top')a=a.filter(x=>x.launched);return a.sort((a,b)=>Number(b.readiness_pct||0)-Number(a.readiness_pct||0))}
function B(x){if(x.launched)return'<span class="badge gold">👑 TOP</span>';if(x.ready)return'<span class="badge good">جاهز</span>';if(x.near_ready)return'<span class="badge gold">قريب</span>';if(Number(x.br.available??x.available)===0)return'<span class="badge red">0 شورت</span>';return''}
function C(x){let av=x.br.available??x.available,ctb=x.br.ctb??x.ctb,reb=x.br.rebate??x.rebate,pr=x.q.price??x.price;return '<div class="card" onclick="this.classList.toggle(\'open\')"><div class="top"><div class="badges"><span class="sym">'+x.symbol+'</span>'+B(x)+'</div><div><span class="price">'+D(pr)+'</span> <span class="score">'+N(x.readiness_pct)+'%</span></div></div><div class="line"><div class="kv"><small>Available</small><b>'+N(av)+'</b></div><div class="kv"><small>القاع</small><b>'+D(x.effective_low)+'</b></div><div class="kv"><small>عن القاع</small><b>'+P(x.effective_distance_pct)+'</b></div><div class="kv"><small>الثبات</small><b>'+(x.effective_sessions??0)+'/4</b></div></div><div class="more line"><div class="kv"><small>Rebate</small><b>'+P(reb)+'</b></div><div class="kv"><small>CTB</small><b>'+P(ctb)+'</b></div><div class="kv"><small>النصف</small><b>'+(x.half_achieved?'✓ ':'')+D(x.half_level)+'</b></div><div class="kv"><small>تاريخ التقسيم</small><b>'+(x.effective_date||'—')+'</b></div><div class="kv"><small>افتتاح التقسيم</small><b>'+D(x.split_open??x.opening_price)+'</b></div><div class="kv"><small>أعلى بعد التقسيم</small><b>'+D(x.highest_since_split)+'</b></div><div class="kv"><small>4H High</small><b>'+D(x.high_4h)+'</b></div><div class="kv"><small>Ready Price</small><b>'+D(x.ready_price)+'</b></div></div></div>'}
function R(){let a=F();count.textContent=a.length+' سهم';cards.innerHTML=a.map(C).join('')||'<div class="empty">لا توجد نتائج</div>';let mix=[...evs];Object.values(halts.rows||{}).forEach(h=>mix.push({symbol:h.symbol,text:'🚨 HALT · '+h.reason,at:(h.halt_date||'')+' '+(h.halt_time||'')}));Object.entries(news.rows||{}).forEach(([s,ls])=>(ls||[]).filter(z=>z.tone==='positive').forEach(z=>mix.push({symbol:s,text:'🟢 '+(z.title||'خبر إيجابي'),at:z.published_at||''})));mix.sort((a,b)=>String(b.at||'').localeCompare(String(a.at||'')));events.innerHTML=mix.slice(0,40).map(e=>'<div class="card"><div class="top"><b>'+e.symbol+'</b><span class="time">'+String(e.at||'').replace('T',' ').slice(0,19)+'</span></div><div class="eventtxt">'+e.text+'</div></div>').join('')||'<div class="empty">لا توجد أحداث بعد</div>'}
async function L(){try{let d=await J('/api/dashboard');snap={rows:d.rows||{}};evs=d.events||[];halts={rows:d.halts||{}};news={rows:d.news||{}};let age=d.health?.heartbeat?Math.max(0,(Date.now()-Date.parse(d.health.heartbeat))/1000):0;hb.textContent='● يعمل · '+Math.round(age)+'ث';R()}catch(e){hb.textContent='● تعذر الاتصال'}}document.querySelectorAll('.f').forEach(b=>b.onclick=()=>{document.querySelectorAll('.f').forEach(z=>z.classList.remove('on'));b.classList.add('on');filter=b.dataset.f;R()});document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>{document.querySelectorAll('.nav button').forEach(z=>z.classList.remove('on'));b.classList.add('on');document.querySelectorAll('section').forEach(z=>z.classList.add('hidden'));document.getElementById(b.dataset.page).classList.remove('hidden')});L();setInterval(L,10000)</script></body></html>"""
