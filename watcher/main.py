...
*{box-sizing:border-box}body{margin:0;background:#071014;color:#edf5f3;font-family:Tahoma,Arial,sans-serif;padding-bottom:76px}.wrap{max-width:720px;margin:auto;padding:12px}.head{display:flex;align-items:center;justify-content:space-between;margin:3px 2px 12px}.logo{font-size:27px;font-weight:900}.logo i{font-style:normal;color:#35e39a}.status{font-size:12px;color:#35e39a}.filters{display:flex;gap:7px;overflow:auto;padding:2px 0 10px;scrollbar-width:none}.f{border:1px solid #1b343a;background:#0b181d;color:#c9d6d4;border-radius:20px;padding:8px 12px;white-space:nowrap}.f.on{background:#35e39a;color:#04100c;border-color:#35e39a;font-weight:800}.card{background:#0b171c;border:1px solid #173038;border-radius:14px;padding:12px;margin-bottom:9px}.row{display:flex;justify-content:space-between;align-items:center;gap:8px}.sym{font-size:19px;font-weight:900}.score{font-size:17px;font-weight:900;color:#35e39a}.price{font-size:15px}.badge{font-size:11px;padding:4px 7px;border-radius:8px;background:#12272d;color:#8ea7a5}.gold{color:#f0c46b}.red{color:#ff7e74}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}.kv{background:#081216;border-radius:9px;padding:7px}.kv small{display:block;color:#718a8c;font-size:10px;margin-bottom:4px}.kv b{font-size:12px}.more{display:none;border-top:1px solid #173038;margin-top:10px;padding-top:9px}.card.open .more{display:grid}.events .card{padding:11px}.time{font-size:10px;color:#718a8c}.empty{text-align:center;color:#718a8c;padding:35px}.nav{position:fixed;bottom:0;left:0;right:0;background:#081318eF;border-top:1px solid #173038;display:flex;justify-content:center;z-index:5}.navin{width:min(720px,100%);display:grid;grid-template-columns:1fr 1fr}.nav button{background:none;border:0;color:#718a8c;padding:14px;font-size:14px}.nav button.on{color:#35e39a;font-weight:900}.hidden{display:none}.sectionTitle{font-size:12px;color:#718a8c;margin:4px 2px 10px}.pulse{color:#35e39a}@media(min-width:650px){.cards{display:grid;grid-template-columns:1fr 1fr;gap:9px}.card{margin:0}.events .card{margin-bottom:9px}}</style></head><body><div class="wrap">
<div class="head"><div class="logo"><i>〽</i> نبض</div><div id="hb" class="status">● جاري الاتصال</div></div>
<section id="radar"><div class="filters"><button class="f on" data-f="all">الكل</button><button class="f" data-f="ready">🎯 الأجهز</button><button class="f" data-f="near">قريب</button><button class="f" data-f="zero">🔥 0 شورت</button><button class="f" data-f="momentum">⚡ لحظي</button><button class="f" data-f="top">👑 TOP</button></div><div id="count" class="sectionTitle"></div><div id="cards" class="cards"></div></section>
<section id="pulse" class="hidden events"><div class="sectionTitle">🔔 آخر الأحداث · HALT · الأخبار الإيجابية</div><div id="events"></div></section></div>
<div class="nav"><div class="navin"><button class="on" data-page="radar">🎯 الرادار</button><button data-page="pulse">🔔 النبض</button></div></div>
<script>
let snap={},evs=[],halts={},news={},filter='all';const N=v=>v==null?'—':Number(v).toLocaleString('en-US',{maximumFractionDigits:2}),P=v=>v==null?'—':Number(v).toFixed(2)+'%',D=v=>v==null?'—':'$'+Number(v).toFixed(Number(v)<1?4:2);
async function J(x){let r=await fetch(x,{cache:'no-store'});if(!r.ok)throw Error(r.status);return r.json()}
function all(){return Object.values(snap.rows||{}).map(x=>({...x,...(x.signal||{}),q:x.price||{},br:x.borrow||{}}))}
function filtered(){let a=all();if(filter==='ready')a=a.filter(x=>x.ready);if(filter==='near')a=a.filter(x=>x.near_ready&&!x.ready);if(filter==='zero')a=a.filter(x=>Number(x.br.available??x.available)===0);if(filter==='momentum')a=a.filter(x=>x.ignition?.fresh);if(filter==='top')a=a.filter(x=>x.launched);return a.sort((a,b)=>Number(b.readiness_pct||0)-Number(a.readiness_pct||0))}
function tag(x){if(x.launched)return '<span class="badge gold">👑 TOP</span>';if(x.ready)return '<span class="badge">🎯 جاهز</span>';if(x.near_ready)return '<span class="badge gold">قريب</span>';if(Number(x.br.available??x.available)===0)return '<span class="badge red">0 شورت</span>';return ''}
function card(x){let av=x.br.available??x.available,ctb=x.br.ctb??x.ctb,reb=x.br.rebate??x.rebate,pr=x.q.price??x.price;return '<div class="card" onclick="this.classList.toggle(\'open\')"><div class="row"><div><span class="sym">'+x.symbol+'</span> '+tag(x)+'</div><div class="score">'+N(x.readiness_pct)+'%</div></div><div class="row" style="margin-top:5px"><b class="price">'+D(pr)+'</b><span class="badge">'+(x.effective_sessions??0)+'/4 ثبات</span></div><div class="grid"><div class="kv"><small>Available</small><b>'+N(av)+'</b></div><div class="kv"><small>القاع بعد التقسيم</small><b>'+D(x.effective_low)+'</b></div><div class="kv"><small>عن القاع</small><b>'+P(x.effective_distance_pct)+'</b></div><div class="kv"><small>Rebate</small><b>'+P(reb)+'</b></div><div class="kv"><small>CTB</small><b>'+P(ctb)+'</b></div><div class="kv"><small>النصف</small><b>'+(x.half_achieved?'✓ ':'')+D(x.half_level)+'</b></div></div><div class="more grid"><div class="kv"><small>تاريخ التقسيم</small><b>'+(x.effective_date||'—')+'</b></div><div class="kv"><small>افتتاح التقسيم</small><b>'+D(x.split_open??x.opening_price)+'</b></div><div class="kv"><small>أعلى بعد التقسيم</small><b>'+D(x.highest_since_split)+'</b></div><div class="kv"><small>4H High</small><b>'+D(x.high_4h)+'</b></div><div class="kv"><small>Ready Price</small><b>'+D(x.ready_price)+'</b></div><div class="kv"><small>⚡ لحظي</small><b>'+(x.ignition?.fresh?('+'+P(x.ignition.pct)):'—')+'</b></div></div></div>'}
function render(){let a=filtered();count.textContent=a.length+' سهم';cards.innerHTML=a.map(card).join('')||'<div class="empty">لا توجد أسهم في هذا الفلتر</div>';let mix=[...evs];Object.values(halts.rows||{}).forEach(h=>mix.push({symbol:h.symbol,text:'🚨 HALT · '+h.reason,at:(h.halt_date||'')+' '+(h.halt_time||'')}));Object.entries(news.rows||{}).forEach(([s,ls])=>(ls||[]).filter(z=>z.tone==='positive').forEach(z=>mix.push({symbol:s,text:'🟢 '+(z.title||'خبر إيجابي'),at:z.published_at||''})));mix.sort((a,b)=>String(b.at||'').localeCompare(String(a.at||'')));events.innerHTML=mix.slice(0,40).map(e=>'<div class="card"><div class="row"><b>'+e.symbol+'</b><span class="time">'+String(e.at||'').replace('T',' ').slice(0,19)+'</span></div><div style="margin-top:6px">'+e.text+'</div></div>').join('')||'<div class="empty">لا توجد أحداث بعد</div>'}
async function load(){try{let [h,s,e,ha,nw]=await Promise.all([J('/health'),J('/snapshot'),J('/events'),J('/halts'),J('/news')]);snap=s;evs=e.events||[];halts=ha;news=nw;hb.textContent='● المحرك يعمل · '+Math.round(h.heartbeat_age_seconds||0)+'ث';render()}catch(e){hb.textContent='● تعذر الاتصال'}}
document.querySelectorAll('.f').forEach(b=>b.onclick=()=>{document.querySelectorAll('.f').forEach(z=>z.classList.remove('on'));b.classList.add('on');filter=b.dataset.f;render()});document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>{document.querySelectorAll('.nav button').forEach(z=>z.classList.remove('on'));b.classList.add('on');document.querySelectorAll('section').forEach(z=>z.classList.add('hidden'));document.getElementById(b.dataset.page).classList.remove('hidden')});load();setInterval(load,10000)
</script></body></html>"""
